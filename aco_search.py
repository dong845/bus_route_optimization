...
class Ant():
    def __init__(self, id, alpha, beta, distance_matrix):
        self.id = id
        self.alpha = alpha
        self.beta = beta
        self.epsilon = 0.5
        ...
        for i in range(len(nodes)):
            if self.city_status[int(nodes[i])]:
                ...
                if (len(self.path) >= 3 and self.criter1() and self.criter2() and self.criter3()) or (len(self.path) < 3 and self.criter1() and self.criter2()):
                    ...
                    probs[i] = pow(score, self.alpha)*pow(1.0 / self.distance_matrix[int(self.current_pos), int(nodes[i])], self.beta)
                    if probs[i]<0:
                        probs[i]=0
                    total_prob += probs[i]
                self.path.pop()
        ...
        if num_points > 0 and True in judge_list:
            if cur_val <= self.epsilon:
                ...
                if p_sums>0:
                    ...
                    next_city = np.random.choice(list(range(len(probs))))
            else:
...
class ACO():
    def __init__(self, ant_num, max_iter, rho, Q, rep_mat):
        self.ant_num = ant_num
        self.max_iter = max_iter
        self.distance_matrix = rep_mat
        self.rho = rho
        self.Q = Q
        self.ants = [Ant(ID, 1, 2, rep_mat) for ID in range(self.ant_num)]
        self.best_ant = Ant(-1, 1, 2, rep_mat)
    ...
    def update_pheromone(self):
        ...
        for ant in self.ants:
            for i in range(1, len(ant.path)):
                temp_pheromone[int(ant.path[i-1]), int(ant.path[i])] += ant.total_rep/ant.move_count
                temp_pheromone[int(ant.path[i]), int(ant.path[i-1])] = temp_pheromone[int(ant.path[i-1]), int(ant.path[i])]
        for i in range(num_points):
            for j in range(num_points):
                self.distance_matrix[i, j] = (1-self.rho)*self.distance_matrix[i, j]+temp_pheromone[i, j]*self.rho
        self.ants = [Ant(ID, 1, 2, self.distance_matrix)
        ...
        while self.iter < self.max_iter:
            ...
            for ant in self.ants:
                ...
                if judge_info and ant.overall_score > self.best_ant.overall_score:
                    print("path:", ant.path)
                    print("total score:", ant.overall_score)
...
aco = ACO(100, 200, 0.7, 100, rep_mat)
path, overall_score = aco.search_path()
print("path:", path)
